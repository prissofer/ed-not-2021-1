...
/*
    ...
    d -> parâmetro da subfunção (vetor) (*poderia não ter pois tem a mesma função de b)
    e -> parâmetro da subfunção (posição inicial do subvetor)
    f -> valor inicial igual à e, posição do menor valor no subvetor
    g -> contador do for da subfunção, com ela no e + 1 e vai até o último valor de d
...
const a = (b, fnComp) => {  
    const c = (d, e) => { 
        let f = e 
        //for(let g = e + 1; g < d.length; g++) if(d[g] < d[f]) f = g (Toda vez que o sinal for menor temos que inverter a posição, e após substuir o sinal por virgula)
        for(let g = e + 1; g < d.length; g++) if(fnComp(d[f], d[g])) f = g
        return f
    }
    for(let h = 0; h < b.length - 1; h++) {
        let i = c(b, h + 1)
        //if(b[h] > b[i]) [b[h], b[i]] = [b[i], b[h]]  // nesse caso não foi necessário inverter pois o sinal está maior e por isso foi somente substituido por , 
...
/*
    4. Teste o algoritmo no arquivo dados/candidatos-2018.mjs, ordenando primeiramente por SG-UE e depois por NR_CANDIDATO.
*/
a(candidatos, (a,b) => {
    if(a.SG_UE === b.SG_UE) {
