...
class Node {
    constructor(val)    {
        ...
        this.next = null        // Ponteiro para o próximo nodo (último)
    }
}
...
export class LinkedList {
    ...
    // Cria no modo para abrigar o valor e o endereço do próximo nodo
    ...
    remove(pos) {
        ...
        if(pos === 0)  {
            removed = this.#head
            this.#head = this.#head.next   // Ou: removed.next por que igualamos ele na linha anterior

            // Atualizando o #tail em caso de remoção do último nodo
            if(this.#count === 1) this.#tail = this.#head.next   // Ou this.#head.null
    }
    ...
    // QUalquer metodo de remoção tem que retornar algo
    removeHead(pos) {
